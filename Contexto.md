...
1. Consultar estado real en la DIAN para facturas `queued` (hay `GET /documents` y sección "Queries and Status" en MATIAS). Hoy `procesarFactura` re-emite en vez de consultar.
2. **Notas crédito** (anulación/devolución). MATIAS soporta "Notas Crédito/Débito"; `POST /invoice` acepta `type_document_id: 5` (falta confirmar contrato: `billing_reference` con CUFE original + `discrepancy_response`). Requiere tabla, UI en detalle de venta, reposición de stock, ticket NC. Esfuerzo grande.
