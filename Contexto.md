...
**2026-09-14 — fix: bug del sync Turso fallido tumbando la app — RESUELTO.** Se aplicó el fix propuesto en la entrada de abajo (2026-09-07).
- `db.cjs`: `_abrirCliente()` ahora distingue dos tipos de error al abrir con `syncUrl`: (a) estado local inválido (ya existía, se recupera bajando la réplica de nuevo) y (b) error de red/TLS/DNS (`error trying to connect`, `invalid peer certificate`, `ENOTFOUND`, `ECONNREFUSED`, `ETIMEDOUT`, `EAI_AGAIN`, nuevo) — ante (b) reintenta `createClient` **sin** `syncUrl`/`authToken` (modo 100% local) en vez de dejar que la excepción se propague y tumbe el proceso principal. Se guarda el motivo en `syncDegradado`/`syncDegradadoMotivo` (nuevas variables de módulo).
- Job de sondeo (`_sondearReconexionTurso`, cada 5 min mientras esté degradado): prueba conectividad contra Turso con un cliente aparte (`SELECT 1`, mismo patrón que ya usaba `set-turso-config`). Deliberadamente **no** reabre el cliente principal en caliente (podría pisar una venta escribiéndose en ese instante) — solo marca `syncReconectable = true` y loguea; reactivar el sync de verdad queda para el próximo reinicio, que ya era el mecanismo existente del proyecto para aplicar cambios de conexión Turso (botón "Reiniciar app" en Ajustes).
- `getSyncStatus()` nuevo (exportado desde `db.cjs`) para que `get-turso-status` (`ipcHandlers.cjs`) devuelva el estado siempre actualizado, no un valor congelado al arrancar — necesario porque `syncDegradado`/`syncReconectable` cambian en runtime y un `require` con destructuring no vería esos cambios.
- UI: banner de Ajustes → "Conexión a la Nube (Turso)" ahora tiene un tercer estado (antes solo activo/sin-sync): degradado (rojo, explica que sigue local sin perder nada) y reconectable (ámbar, invita a usar el botón "Reiniciar app" que ya existía en la misma tarjeta). Banner nuevo equivalente en `FacturacionDIAN.tsx` aclarando que facturar no depende de esto (va por MATIAS).
- **Verificado en runtime real, no solo en teoría:** esta misma máquina de desarrollo tiene el proxy que intercepta TLS que originó el bug. Se corrió `npm run electron:dev` con credenciales Turso reales en el `.env` y se reprodujo el error exacto documentado (`invalid peer certificate: UnknownIssuer`) — con el fix aplicado, la app lo logueó y siguió arrancando normal (`Base de datos inicializada correctamente` → job DIAN iniciado), sin crashear. Antes del fix esto mataba el proceso. `tsc --noEmit` y `eslint` limpios (sin errores nuevos, quedan los `any` preexistentes de antes de esta sesión).
- Pendiente a futuro (no bloqueante): si se quiere reactivar el sync sin pedirle un restart al usuario, habría que sincronizar el hot-swap del cliente con un lock que impida escrituras concurrentes durante el intercambio — se descartó por ahora por riesgo/beneficio en una app que maneja plata.

**2026-09-07 — bug conocido (SIN arreglar): un sync Turso fallido tumba toda la app.** Al pegar credenciales Turso válidas en el `.env` y arrancar en una red con **proxy que intercepta TLS** (típico de redes corporativas/gubernamentales), la app crashea en el arranque con `Uncaught Exception: sync error: error trying to connect: invalid peer certificate: UnknownIssuer` (`db.cjs:95`, dentro de `createClient`). Causa: el cliente nativo de libSQL trae su propio bundle de certificados y no mira el almacén de Windows, así que rechaza el cert re-firmado por el proxy. `_abrirCliente()` en `db.cjs` solo captura corrupción de réplica local (`invalid local state`, etc.), no errores de conexión/TLS/DNS — cualquier fallo de red al abrir con `syncUrl` se propaga como excepción no capturada y mata el proceso principal, en contra del diseño offline-first ("si no hay conexión, corre local, no bloquea nada").
- **Nota conceptual:** facturar NO depende de Turso. La facturación electrónica va por la API de MATIAS (`sandbox-api.matias-api.com`), servicio HTTPS aparte. Turso es solo la base en la nube (backup/sync).
- **Workaround actual:** sacar (o comentar) `TURSO_DATABASE_URL` y `TURSO_AUTH_TOKEN` del `.env` → `syncEnabled = false`, la app corre 100% local sin sync y no crashea. O usar una red sin ese proxy. Para tener sync real hay que pedirle a sistemas que agregue `*.turso.io` a las excepciones del proxy (que no lo inspeccione).
- **Fix propuesto (pendiente):** en `_abrirCliente()`, ante un error de conexión/TLS al abrir con `syncUrl`, reintentar `createClient` **sin** `syncUrl`/`authToken` (modo local puro), marcar un flag, avisar en la UI (Ajustes / Facturación DIAN) y dejar que el job de reintento intente el sync más tarde.
- **Dato de contexto:** el Git Credential Manager de la máquina donde salió el error ya tiene la verificación TLS deshabilitada (`SECURITY WARNING` al pushear) — o sea, esa red efectivamente hace MITM de HTTPS y alguien ya lo destrabó para `git` bajando la seguridad. Mismo origen que el error de libSQL.
