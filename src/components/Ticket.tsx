...
export default function Ticket({ id, fecha, items, total, subtotal, impuestos }: TicketProps) {
    return (
        <div className="print-only text-slate-900 font-mono text-xs w-[80mm] mx-auto p-4 bg-white">
            <div className="text-center mb-6 space-y-1">
                ...
                <div className="border-y border-dashed border-slate-300 py-2 my-2">
                    <p className="font-bold">TICKET DE VENTA #{id}</p>
            ...
            <div className="space-y-2 mb-6">
                ...
                {items.map((item, i) => (
                    <div key={i} className="flex justify-between items-start gap-1">
                        ...
                        <span className="w-1/3 text-right font-bold">${item.subtotal?.toLocaleString('es-AR') || (item.precio_venta * item.quantity).toLocaleString('es-AR')}</span>
                    </div>
                ))}
            </div>
