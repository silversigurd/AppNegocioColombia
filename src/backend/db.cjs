...
// Determinamos la ruta de la base de datos (se guardará en un archivo local .sqlite)
    ...
    db.serialize(() => {
      ...
      FOREIGN KEY(venta_id) REFERENCES Ventas(id)
        ...
        db.get('SELECT COUNT(*) as count FROM Sucursales', (err, row) => {
            if (row && row.count === 0) {
                db.run(`INSERT INTO Sucursales (nombre, direccion) VALUES ('Sede Principal', 'Dirección por defecto')`);
            }
        });
    });
}
